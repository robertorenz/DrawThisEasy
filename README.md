...
[![Latest release](https://img.shields.io/github/v/release/robertorenz/DrawThisEasy?label=download&color=0EA5E9&v=1.2.3)](https://github.com/robertorenz/DrawThisEasy/releases/latest)
...
- Presentation: three more transitions — **pan then zoom**, **rotate & zoom**, **spring zoom** — plus a **Random** setting that varies the transition on every screen.
...
- **Presentation mode** — turn a diagram into a slideshow. **Present** opens a panel where you mark screens (capture the current view, or auto-add empty ones), **double-click a screen to rename it**, **update a screen to the current view** (◎ — re-captures its position & zoom), reorder them in a numbered queue, choose a background color, and pick a **transition style** — *zoom out & in*, *smooth glide*, *pan then zoom*, *rotate & zoom*, *spring zoom*, *fade through background*, *instant cut*, or *random* (a different one each screen). **Start Presenting** goes fullscreen and flies between screens. Navigate with arrow keys (**↑/← back**, **↓/→ forward**), **Space/Enter** (forward), **Home/End** (first/last); **Esc** exits.
