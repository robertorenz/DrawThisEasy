...
public static class L10n
{
    ...
    private static readonly Dictionary<Language, Dictionary<string, string>> _strings = new()
    {
        [Language.En] = new()
        {
            ...
            ["present.goto"]         = "Click to preview · double-click to rename",
            ["present.rename"]       = "Double-click to rename",
            ["present.recapture"]    = "Set to current view (position + zoom)",
            ["present.up"]           = "Move up",
            ["present.down"]         = "Move down",
            ["present.delete"]       = "Remove screen",
            ["present.bg"]           = "Background",
            ["present.transition"]   = "Transition",
            ["present.tx.zoom"]      = "Zoom out & in",
            ["present.tx.glide"]     = "Smooth glide",
            ["present.tx.panzoom"]   = "Pan, then zoom",
            ["present.tx.spin"]      = "Rotate & zoom",
            ["present.tx.spring"]    = "Spring zoom",
            ["present.tx.whirl"]     = "Whirlwind",
            ["present.tx.cut"]       = "Instant cut",
            ["present.tx.fade"]      = "Fade through background",
            ["present.tx.random"]    = "Random (varies each screen)",
            ...
            ["manual.present.h"] = "Presentation mode",
            ["manual.present.p"] = "Turn a diagram into a slideshow. Click Present in the toolbar to open the panel, build a series of numbered screens, then Start Presenting to fly between them fullscreen. Spreading scenes across the canvas (see Free Space) makes the zoom-out transition reveal the whole map before diving into the next one.",
            ["manual.present.b.mark"] = "Mark current view captures whatever is framed on screen as the next numbered screen.",
            ["manual.present.b.free"] = "Free Space (toolbar, or Add empty screen in the panel) finds an empty gap on the canvas and adds it as the next screen — lay out each scene in its own area.",
            ["manual.present.b.manage"] = "In the panel: double-click a screen to rename it, ▲ / ▼ to reorder, ◎ to set it to the current view (position + zoom), ✕ to remove; click a row to preview it.",
            ["manual.present.b.style"] = "Pick a background color and a transition: zoom out & in, smooth glide, pan then zoom, rotate & zoom, spring zoom, whirlwind, fade through background, instant cut, or Random (a different one each screen).",
            ["manual.present.b.start"] = "Start Presenting goes fullscreen with your background; a corner label shows the current screen name and position (e.g. \"2 / 5\").",
            ["manual.present.b.keys"] = "Navigate: → ↓ Space Enter go forward, ← ↑ go back, Home / End jump to first / last, Esc exits.",
        ...
        [Language.Es] = new()
        {
            ...
            ["present.goto"]         = "Clic para previsualizar · doble clic para renombrar",
            ["present.rename"]       = "Doble clic para renombrar",
            ["present.recapture"]    = "Establecer a la vista actual (posición + zoom)",
            ["present.up"]           = "Subir",
            ["present.down"]         = "Bajar",
            ["present.delete"]       = "Quitar pantalla",
            ["present.bg"]           = "Fondo",
            ["present.transition"]   = "Transición",
            ["present.tx.zoom"]      = "Alejar y acercar",
            ["present.tx.glide"]     = "Deslizamiento suave",
            ["present.tx.panzoom"]   = "Desplazar, luego acercar",
            ["present.tx.spin"]      = "Rotar y acercar",
            ["present.tx.spring"]    = "Zoom con rebote",
            ["present.tx.whirl"]     = "Torbellino",
            ["present.tx.cut"]       = "Corte instantáneo",
            ["present.tx.fade"]      = "Fundido a través del fondo",
            ["present.tx.random"]    = "Aleatorio (varía en cada pantalla)",
            ...
            ["manual.present.h"] = "Modo presentación",
            ["manual.present.p"] = "Convierte un diagrama en una presentación. Haz clic en Presentar en la barra de herramientas para abrir el panel, crea una serie de pantallas numeradas y luego Iniciar presentación para volar entre ellas en pantalla completa. Distribuir las escenas por el lienzo (ver Espacio libre) hace que la transición de alejar muestre todo el mapa antes de acercarse a la siguiente.",
            ["manual.present.b.mark"] = "Marcar vista actual captura lo que esté encuadrado en pantalla como la siguiente pantalla numerada.",
            ["manual.present.b.free"] = "Espacio libre (barra de herramientas, o Agregar pantalla vacía en el panel) encuentra un hueco vacío en el lienzo y lo agrega como la siguiente pantalla, para componer cada escena en su propia área.",
            ["manual.present.b.manage"] = "En el panel: doble clic en una pantalla para renombrarla, ▲ / ▼ para reordenar, ◎ para establecerla a la vista actual (posición + zoom), ✕ para quitarla; haz clic en una fila para previsualizarla.",
            ["manual.present.b.style"] = "Elige un color de fondo y una transición: alejar y acercar, deslizamiento suave, desplazar luego acercar, rotar y acercar, zoom con rebote, torbellino, fundido a través del fondo, corte instantáneo o Aleatorio (una distinta en cada pantalla).",
            ["manual.present.b.start"] = "Iniciar presentación pasa a pantalla completa con tu fondo; una etiqueta en la esquina muestra el nombre y la posición de la pantalla actual (p. ej. «2 / 5»).",
            ["manual.present.b.keys"] = "Navegación: → ↓ Espacio Entrar avanzan, ← ↑ retroceden, Inicio / Fin van a la primera / última, Esc sale.",
